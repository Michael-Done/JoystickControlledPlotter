...
*/
void setSpeed(int targetXSpeed, int targetYSpeed, const struct constants *CON){
    if(targetXSpeed > 0 && nMotorEncoder[X_AXIS] >= CON->X_LIMIT)
        targetXSpeed = 0;
    else if(targetXSpeed < 0 && nMotorEncoder[X_AXIS] <= 0)
        targetXSpeed = 0;

    if(targetYSpeed > 0 && nMotorEncoder[Y_AXIS] >= CON->Y_LIMIT)
        targetYSpeed = 0;
...
/*  void moveTo : will move the drawing head to a specified x and y position
    Parameters:
        ...
        const struct constants *CON  : structure containing all constants specific to the physical robot
*/
void moveTo(int x, int y, int speed, const struct constants *CON) {
    while(abs(nMotorEncoder[X_AXIS]-x) > CON->MOVE_TO_TOL && abs(nMotorEncoder[Y_AXIS]-y) > CON->MOVE_TO_TOL)){
...
/*  void actuate: actuates the drawing head to be in the "drawing" or "not drawing" position
    Parameters:
        ...
        const struct constants *CON  : structure containing all constants specific to the physical robot
*/
void actuate(bool dir, const struct constants *CON){
    // TO DO
}